...
#         """Callback function for the timer."""
#         if self.offboard_setpoint_counter == 5:
#             self.engage_manual_mode()  # Теперь включаем Manual Mode
#             self.arm()
#             self.ready_flag = True
#
#         if self.ready_flag and self.manual_mode_enabled:
#             # В Manual Mode просто выводим текущие скорости, не публикуя их
#             self.get_logger().info(f"Manual mode: velocity setpoints vx: {self.velocity_vx}, vy: {self.velocity_vy}, vz: {self.velocity_vz}, angular_z: {self.angular_z}")
#             # Публикуем скорость в другой топик, если требуется:
#             # self.publish_velocity_setpoint(self.velocity_vx, self.velocity_vy, self.velocity_vz, self.angular_z)
#
#         if self.offboard_setpoint_counter < 6:
#             self.offboard_setpoint_counter += 1
#
